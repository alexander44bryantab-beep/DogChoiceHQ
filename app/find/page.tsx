...
export default function FindPage() {
  ...
  }).sort((a, b) => b.score - a.score), [profile]);
  ...
  return (
    <main className="find-page">
      <div className="container find-wrap">
        ...
        <section className="results"><div className="results-head"><div><p className="eyebrow">YOUR STARTING POINT</p><h2>Matched products</h2></div><span>Demo catalog</span></div>
          {recommendations.map(({ product, score, reasons }, index) => <article className="match-card" key={product.id}><div className="rank">#{index + 1}</div><div className="match-main"><p className="brand-small">{product.brand}</p><h3>{product.name}</h3><p>{product.bestFor}</p><div className="reason-list">{reasons.map((reason) => <span key={reason.label}>{reason.label}{reason.points ? ` +${reason.points}` : ""}</span>)}</div></div><div className="match-score"><strong>{score}</strong><span>match</span></div></article>)}
        </section>
